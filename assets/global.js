...
// ===== Lucent Reveal Animation =====
(function () {
  const SELECTOR = ".lucent-reveal";

  // 既に対応済みなら二重初期化しない
  if (window.__lucentRevealInitialized) return;
  window.__lucentRevealInitialized = true;

  const els = Array.from(document.querySelectorAll(SELECTOR));
  if (!els.length) return;

  // Reduce Motion の場合は即表示
  const reduce =
    window.matchMedia &&
  ...
  // Shopify Theme Editor の動的再描画にも一応追随
  document.addEventListener("shopify:section:load", (e) => {
    const container = e && e.target ? e.target : document;
    const newEls = Array.from(container.querySelectorAll(SELECTOR));
    newEls.forEach((el) => {
